    ...
    <div className={styles.wrapper}>
      <section className={styles.card}>
        ...
        <dl className={styles.grid}>
          {([['APR',sla.apr!==null?sla.apr+'%':null],['Monthly Payment',sla.monthly_payment!==null?formatCurrency(sla.monthly_payment, price_estimate?.currency):null],['Term',sla.term!==null?sla.term+' months':null],['Residual Value',sla.residual_value!==null?formatCurrency(sla.residual_value, price_estimate?.currency):null],['Mileage Limit',sla.mileage_limit!==null?sla.mileage_limit.toLocaleString()+' km':null],['Penalties',sla.penalties]]as[string,string|null][]).map(([k,v])=>(
      ...
      {price_estimate&&<section className={styles.card}><h3 className={styles.sec}>Total Lease Value</h3><p className={styles.bigNum}>{formatCurrency(price_estimate.market_value, price_estimate.currency)}</p><div className={styles.conf}><span className={styles.confLbl}>Confidence</span><div className={styles.confBar}><div className={styles.confFill} style={{width:price_estimate.confidence+'%'}}/></div><span className={styles.confPct}>{price_estimate.confidence}%</span></div></section>}
