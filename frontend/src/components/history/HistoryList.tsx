      ...
      {items.map(item=>(
        <li key={item.job_id} className={styles.item} onClick={()=>onSelect(item.job_id)}>
          <div className={styles.info}>
            ...
            {item.fairness_score!==null&&<span className={styles.score} style={{color:item.fairness_score>=70?'var(--success)':item.fairness_score>=40?'var(--warning)':'var(--danger)'}}>{item.fairness_score}/100</span>}
          </div>
          <button className={styles.del} onClick={e=>handleDelete(item.job_id,e)} title="Delete">x</button>
        </li>
      ))}
    </ul>
  )
